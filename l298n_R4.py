...
def forward(speed):
    ...
    try:
        ...
        display.lcd_display_string(f"Motor speed{speed_1}: {distance}", 2)
    except:
        print("LCD Failed <--FORWARD-->")
        print(f"Motor speed{speed_1}: {distance}", 2)
...
def reverse(speed):
    ...
    try:
        display.lcd_clear()
        display.lcd_display_string(f"<--REVERSE-->")
        display.lcd_display_string(f"Motor speed{speed_1}")
    except:
        print("LCD Failed <--REVERSE-->")
        print(f"Motor speed{speed_1}:{distance}")
    motor_2_speed.ChangeDutyCycle(speed_1)
    motor_1_speed.ChangeDutyCycle(speed_1)
